...
public class HasilCincinBolaView extends JFrame {

    public HasilCincinBolaView(CincinBola cb) {
        initComponents(cb);
        setLocationRelativeTo(null);
        setTitle("Hasil Kalkulasi Cincin Bola");
    }

    private void initComponents(CincinBola cb) {
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setSize(500, 450);
        setLayout(null);

        JLabel jLabelTitle = new JLabel("HASIL CINCIN BOLA", SwingConstants.CENTER);
        ...
        JTextField jTextFieldVolume = new JTextField(String.format("%.2f", cb.getVolumeCincinBola())); //
        ...
        JTextField jTextFieldLuas = new JTextField(String.format("%.2f", cb.getLuasPermukaanCincinBola())); //
        ...
        jButtonEdit.addActionListener(e -> {
            dispose();
            new CincinBolaView(cb).setVisible(true);
        });
