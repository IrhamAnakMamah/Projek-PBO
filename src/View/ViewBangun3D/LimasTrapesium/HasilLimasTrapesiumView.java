...
public class HasilLimasTrapesiumView extends JFrame {

    public HasilLimasTrapesiumView(LimasTrapesium limas) {
        initComponents(limas);
        setLocationRelativeTo(null);
        setTitle("Hasil Kalkulasi Limas Trapesium");
    }

    private void initComponents(LimasTrapesium limas) {
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setSize(500, 450);
        setLayout(null);

        JLabel jLabelTitle = new JLabel("HASIL LIMAS TRAPESIUM", SwingConstants.CENTER);
        ...
        JTextField jTextFieldVolume = new JTextField(String.format("%.2f", limas.getVolumeLimasTrapesium())); //
        ...
        JTextField jTextFieldLuas = new JTextField(String.format("%.2f", limas.getLuasPermukaanLimasTrapesium())); //
        ...
        jButtonEdit.addActionListener(e -> {
            dispose();
            new LimasTrapesiumView(limas).setVisible(true);
        });
